...
if(isset($_GET['id']))
{
    ...
    if($result=mysqli_query($conn,$query)) {
      ...
      {
        $upquery="UPDATE `categories_level1` SET `product_filter_sidebar_status`=0 WHERE `cat_id`=".$_GET['id'];

      }
      else{
        $upquery="UPDATE `categories_level1` SET `product_filter_sidebar_status`=1 WHERE `cat_id`=".$_GET['id'];
