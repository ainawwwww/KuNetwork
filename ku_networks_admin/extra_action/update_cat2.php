...
if(isset($_POST['update_cat'])){
    ...
    {
        $updatequery_woImg="UPDATE `categories_level2` SET `name`='".$_POST['category']."',`product_filter_sidebar_status`='".$_POST['sidebar_status']."',`menu_status`='".$_POST['menu_status']."' WHERE `id`=".$_POST['lastid'];
        mysqli_query($conn,$updatequery_woImg);   
                            echo "<script>
                            alert('Category Has Been Updated Succesfully');
                            window.location='../categoryleveltwo.php';
                            </script>"; 
    ...
    $uploadStatus=0; 
           ...
           if($uploadStatus==1 )
           {
                        ...
                        if (move_uploaded_file($fileTmpName, $uploadPath)) {
                            ...
                            $upload_query="UPDATE `categories_level2` SET `name`='$cat_name',`image`='$newFileName',`product_filter_sidebar_status`='$sidebar_status',`menu_status`='$menu_status' WHERE `id`=".$_POST['lastid'];
