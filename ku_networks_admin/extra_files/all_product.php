...
if (isset($_SESSION['role']) && $_SESSION['role']==1){
  $fetch="UPDATE `products` SET `read_status`=1 WHERE read_status=0";
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
      ...
      if(mysqli_num_rows($sql)>0){
        ...
        WHERE 
            p.id_p = ".$row['id_p']." -- Replace '?' with the specific product ID
        LIMIT 1";
      ...
      ?>
       ...
       <td><a href="product_complete_detail.php?id=<?=$row['id_p']?>"><?=$row['product_name']?></a></td>
           <td><a class="btn btn-info" href="product_images.php?id=<?=$row['id_p']?>">Images</a></td>
           <td><del>AED <?=$row['product_price']?> / </del><span>AED <?=$row['discount_price']?></span></td>
           <td><?=$row['Badge']?></td>
           <td><?=$row['stock']?></td>
           <td><?=$row['created_at']?></td>
           <td>
              <div class="d-flex">
              <a href="product_detail.php?id=<?=$row['id_p']?>"><button class="btn btn-warning btn-sm mx-2"  style="border-radius:15px;"  ><i class="fas fa-eye"></i></button></a>
                <a href="add_pro_detail.php?id=<?=$row['id_p']?>"><button class="btn btn-success btn-sm mx-2"  style="border-radius:15px;"  ><i class="fas fa-plus"></i></button></a>
                
            </div>
           </td>
           <td>
              <div class="d-flex">
               <a href="update_product_form.php?id=<?=$row['id_p']?>"><button class="btn btn-info btn-sm mx-2"  style="border-radius:15px;"  ><i class="fas fa-pencil-alt"></i></button></a>
               
               <form method="POST" action="action/delete_product.php">
